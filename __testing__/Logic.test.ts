...
describe('formatStats function', () => {
  ...
  const stats = ['The quick brown fox jumped over the lazy dog', 4000, { 1: err }, {}, [1000, 1000, 1000, 1000, 0]];
  ...
  test('Should properly format fresh stats from Typer component', () => {
    ...
    expect(formatted[3]).toBe(stats[4]);
    expect(formatted[4]).toBe(stats[0]);
    expect(formatted[5]).toBe(stats[3]);
  });

  test('Should properly format stats from database', () => {
    const fromDatabase = { lastwpm: 132, lasterrors: 1, lastaccuracy: 97.73 };
  ...
  test('Given empty data from Typer OR database, should return empty stats', () => {
    const formatted = Logic.formatStats([0]);
    const fromDatabase = Logic.formatStats({});
